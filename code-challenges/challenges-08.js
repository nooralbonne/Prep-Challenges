...
const customerAndAge = (obj) => {
  let newArr = [];
  for (let key in obj) {
    newArr.push(`Customer Name :${key} , Age :${obj[key]}`);
  }
  return newArr;
};
...
const getEntries = (obj) => {
  ...
  {
    info.push(`${key}: ${Array.isArray(obj[key]) ? obj[key].join(',') : obj[key]}`);
...
const getStudents = (arr) => {
  // write your code here
  let std=[];
  for(let studentsName of arr)
  {
    for(let course of courses)
    {
      if (course.Students.includes(studentsName)) {
        std.push({ student: studentsName, course: course.course});
        break;
      }
    }
  }
